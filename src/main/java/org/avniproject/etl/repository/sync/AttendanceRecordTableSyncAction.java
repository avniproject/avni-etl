...
@Repository
public class AttendanceRecordTableSyncAction implements EntitySyncAction {

    private final JdbcTemplate jdbcTemplate;
    private static final String sql = readSqlFile("attendanceRecord.sql.st");

    @Autowired
    public AttendanceRecordTableSyncAction(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    @Override
    public boolean doesntSupport(TableMetadata tableMetadata) {
        return !tableMetadata.getType().equals(TableMetadata.Type.AttendanceRecord);
    }
