...
@Repository
public class AttendanceTypeTableSyncAction implements EntitySyncAction {

    private final JdbcTemplate jdbcTemplate;
    private static final String sql = readSqlFile("attendanceType.sql.st");

    @Autowired
    public AttendanceTypeTableSyncAction(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    @Override
    public boolean doesntSupport(TableMetadata tableMetadata) {
        return !tableMetadata.getType().equals(TableMetadata.Type.AttendanceType);
    }
